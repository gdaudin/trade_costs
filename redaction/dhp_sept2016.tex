...
Let us consider first the trend in total transport costs (ie, considering the upper line in Figure \ref{fig:decomp_TC_3d}). Both air and ocean shipping exhibit a downward trend in overall transport costs since 1974. (-2.1\% a year for mean air transport costs and -2.0\% a year for mean ocean transport costs).
\footnote{One may worry that this result springs from high oil-shock related transport costs in 1974. However, computing the time trends from 1980 does not change dramatically the picture. The yearly trend from 1980 is -2\% for mean air transport costs and -1.6\% for mean ocean transport costs. In order to go as far as possible into the past, this paper will focus on the results from 1974}
 \todo{GD : j'ai amélioré. Qu'en penses-tu ?}
These results are consistent with \citet{hummels2007}.
