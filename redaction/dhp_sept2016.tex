...
\paragraph{The estimated equation} Our purpose is to provide estimates over time of the size of ad-valorem and additive costs among total transport costs. To do so, we start from the equation that expresses the price $p(z)$ of a good $z$ paid by the importer (import, or cif) a function of the producer price $\widetilde{p}(z)$ (export, or fas), given both per-kg ($t$) and ad-valorem ($\tau$) transport costs, according to:

\begin{equation}
p(z) = \tau \widetilde{p}(z) + t \label{eq:base}
\end{equation}

\noindent As usual in the literature, the iceberg trade costs are denoted $\tau$ (with  $\tau \geq 1$, $\tau=1$ meaning no ad-valorem trade costs), while additive trade costs are labeled $t$ (with $t \geq 0$, $t=0$ implying no additive costs).  Let us denote $i$ the origin country, and $k$, the sector. Transforming the above equation (\ref{eq:base}) as ratio, we thus get the following equation at the root of our estimation:\footnote{We skip the year and transport-mode dimensions in the notations for reading convenience.}

\begin{equation*}
\frac{p_{ikz}}{\widetilde{p}_{ikz}} -1 = \tau_{ik} -1 +\frac{t_{ik}}{ \widetilde{p}_{ikz}} \label{eq:base_estimee}
\end{equation*}
...
\begin{equation*}
\frac{p_{ikz}}{\widetilde{p}_{ikz}}-1 =\tau_{i} \times \tau_{k} -1 +\frac{t_{i} + t_{k}}{ \widetilde{p}_{ikz}} \label{eq:theory_equation}
\end{equation*}

The ratio $\frac{p_{ikz}}{\widetilde{p}_{ikz}}$ has a lower bound of one, since by construction, the cif price $p$ cannot be lower than the fas price ($p_{ikz}>\widetilde{p}_{ikz}$). Taking into account this constraint in the estimation suggests that the error term should be always positive and multiplicative, as in:

\begin{equation*}
\frac{p_{ikz}}{\widetilde{p}_{ikz}}-1 =\left(\tau_{i} \times \tau_{k} -1+\frac{t_{i} + t_{k}}{\widetilde{p}_{ikz}} \right)\times \exp(\epsilon_{ikz})
\end{equation*}
...
\begin{equation}
\ln\left(\frac{p_{ikz}}{\widetilde{p}_{ikz}}-1 \right)= \ln \left(\tau_{i} \times \tau_{k}+\frac{t_{i} + t_{k}}{\widetilde{p}_{ikz}}-1 \right) + \epsilon_{ikz} \label{eq:estimatedequation}
\end{equation}
...
\begin{eqnarray}
\ln\left(\frac{p_{ikz}}{\widetilde{p}_{ikz}}-1 \right)&=& \ln \left(\tau_{i} \times \tau_{k}-1 +\frac{t_{i} + t_{k}}{\widetilde{p}_{ikz}}\right) + \epsilon_{ikz} \label{eq:model_with_add} \\
\ln\left(\frac{p_{ikz}}{\widetilde{p}_{ikz}}-1 \right)&=& \ln \left(\tau_{i} \times \tau_{k}-1 \right) + \epsilon^{ice}_{ikz} \label{eq:model_nlI}
\end{eqnarray}
