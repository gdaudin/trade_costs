...
Figure \ref{fig:totalTC_compeffects_excl} reports the results. Panel (a) displays the time changes in the overall transport costs, by transport mode. In Panels (b) and (c), we report the time changes of the ad-valorem costs and the additive costs respectively (re-scaled to start from the reference value 100), for each transport mode.
...
Consider first the time trends in transport costs at the aggregate level (Figure \ref{fig:totalTC_compeffects_excl}). Two main results emerge. First, we do not find evidence of a significant difference between the ``raw'' transport costs and the `pure'' transport costs in Air transport. This result for each of the two components, additive and ad-valorem (Figure \ref{fig:totalTC_compeffects_excl}, left panels). Air transport costs were reduced by 50\% between 1974 and 2013, and this is mainly attributable to a reduction in the ``pure'' transport costs. This stands in contrast with \citet{hummels2007}, which obtained that the ``pure'' transport costs had decreased more (over 1973- 2004) than the unfitted ones, suggesting an important role to composition effects. As noted above, this difference of results can be explained by the difference in methodology. Whereas \citet{hummels2007} attributes all changes that are not changes in the ad-valorem components to composition effects, we show that part (and as a matter of fact, most) of these changes come from a reduction in the ``pure'' transport costs, in their additive dimension. 
