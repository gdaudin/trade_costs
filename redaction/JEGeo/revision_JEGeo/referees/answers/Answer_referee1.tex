...
Nevertheless, we did'nt want to stop at this very first argument. We indeed took the referee's remark very seriously and we did our best to address it to give you a convincing answer. It was not without difficulty yet, in particular due to data constraints. At a more fundamental level, we also believe that looking at transport costs on a per-weight basis do add relevant information. We develop each element of answer in what follows.
...
Integrating these various constraints, we run this robustness check (weight versus quantity) \textit{i)} on a sample reduced over 2009-2019, considering single transport mode flows; all in all, this sample amounts to 66\% of the value of the baseline sample, \textit{ii)} retaining a non-separable structure of three-dimensional fixed effects $t_{is(k)u}$, $\tau_{is(k)u}$ where , according to the following equation:
\begin{equation}
...
Our main objective in this paper is to provide a quantitative assessment of international transport costs over time; on that matter, one may start with the observation that transport cost schedules are expressed historically and currently in a mix per-weight and per-volume basis. \textbf{stop here}
For example, in 2021, air freight trade costs are expressed in weights, with a minimal weight per volume (``volumetric weight’’ or ``dimensional weight). See for example \href{https://transporteca.co.uk/shipping-costs}{https://transporteca.co.uk/shipping-costs}, \href{http://wap.dhl.com/serv/volweight.html}{http://wap.dhl.com/serv/volweight.html} and \href{https://en.wikipedia.org/wiki/Dimensional_weight}{ https://en.wikipedia.org/wiki/Dimensional\_weight}).
