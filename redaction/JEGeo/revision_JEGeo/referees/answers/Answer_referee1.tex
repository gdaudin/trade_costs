...
%Gains de bien-être sont plus forts lorsqu’on bouge les coûts additifs. Voir >Table_compWelfare2.xls.
%Ex 1 : Comparer C (que des add valorem) et E (additifs et ad valorem) : on baisse les coûts de transports ; sheet 1 = air, sheet 2 = Vessel.
%Ex 2 : si en plus on ajuste fx, cela amplifie le diagnostic. Voir colonnes D et F : la baisse des couts fixes joue plus dans un monde avec ad%ditifs que sans additifs.
%Voir « C:\Users\coadministrateur\Dropbox\Papier_Lise_Guillaume\trade_cost\model », et les tableaux afférents.
% Big picture: quantitatively, we are more exhaustive/more complete. We find big welfare differences with additive costs, even if we focus only on transport costs.--> Our results are probably a lower bound.
