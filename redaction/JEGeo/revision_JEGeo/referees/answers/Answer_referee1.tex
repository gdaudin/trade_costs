...
If we take the value of the $\beta$ by itself, there is no clear criteria to discriminate between the value estimated with our method and the one obtained with the suggested method (when, of course, run on the same sample). Further, as shown in Table \ref{tab_comp_referee1_baseline}, no clear conclusion on a potential upwards or downwards bias that would be attached to our method. Things are more clear-cut in terms of accuracy of the estimation. Specifically, our method yields a more precise estimation of the $\beta$ than the referee's method. To develop on this, for each year and transport mode:
    ...
    \begin{center}
    \includegraphics[height=4in]{../../revised_article/comparaison_amplitude_baseline_referee1_2013_ves.png}
...
Nevertheless, we did'nt want to stop at this very first argument. We indeed took the referee's remark very seriously and we investigated this question in depth. It was not without difficulty yet, in particular due to data constraints. At a more fundamental level, we also believe that looking at transport costs on a per-weight basis do add relevant information. We develop each line of argument in what follows.
...
Integrating these various constraints, we run this robustness check (weight versus quantity) \textit{i)} on a sample reduced over 2009-2019, considering single transport mode flows - amounting to 66\% of the value of the baseline sample; \textit{ii)} retaining a non-separable structure of three-dimensional fixed effects $t_{is(k)u}$, $\tau_{is(k)u}$ where , according to the following equation:
\begin{equation}
\ln\left(\frac{p_{ik}}{p^u_{ik}}-1 \right)= \ln \left(\tau_{is(k)u} -1+\frac{t_{is(k)u} }{p^u_{ik}} \right) + \epsilon_{ik} \label{eq:model_IetA}
\end{equation}
\noindent where $p^u_{ik}$ refers to the unit price associated with quantity and $u$ to the unit considered to measure the quantity. Depending on the sector, two different units are considered, kilograms and numbers.

The results are reported in Table \ref{tab_comp_wgt_qty} and in Figure \ref{fig:beta_comp_wgt_qty} (as well as in the online appendix D.4 \textbf{XXX Check in the end XXX}. In Table \ref{tab_comp_wgt_qty}, for each transport mode, the first column reports the estimation based on the price per quantity; while the second column reports the results obtained considering the price per kg on the same sample; the last column reported our baseline results on the large sample (considering the average value over 2005-2019, under the separability assumption).
...
These results show clearly that using price per unit (whatever that unit is) instead of price per weight decreases the share of additive costs (except if one look at the median for air). In this respect, they stand in line with the results of \citet{Lashkaripour_JIE2020}, stressing the correlation between weight per unit and unit price. Still, the share of additive cost is not reduced to zero (except if one looks at the median for vessel).
Thus these results do not disqualify our approach, especially as we cover a much larger period and sample with our baseline estimate.

\subsubsection{Why is price per weight interesting}
It is true that to study welfare, the relevant variable that would create distortions is the cost per unit. \textbf{Je trouve l'argument delicat car dans notre section modele, c'est justement le welfare qu'on regarde.}
However, our primary aim is descriptive.


\textbf{XXX LP: ma version plutot XXXX}
At a more fundamental level, we do believe that the study of transport costs measured on a per-weight add some relevant information on the international transport cost burden faced by exporters. Let us explain the underlying rationale. 
 
Our main objective in this paper is to provide a quantitative assessment of the international transport costs burden over time; on that matter, one may start with the observation that transport cost schedules are expressed through a mix per-weight and per-volume basis, both historically and nowadays. For example, in 2021, air freight trade costs are expressed in weights, with a minimal weight per volume (``volumetric weight'' or ``dimensional weight'')\footnote{See for example \href{https://transporteca.co.uk/shipping-costs}{https://transporteca.co.uk/shipping-costs}, \href{http://wap.dhl.com/serv/volweight.html}{http://wap.dhl.com/serv/volweight.html} and \href{https://en.wikipedia.org/wiki/Dimensional_weight}{ https://en.wikipedia.org/wiki/Dimensional\_weight}).} while maritime freight costs are expressed in volume.
Hence, looking at per-kg transport cost can be interpreted straightforwardly as trying to estimate the actual existing shipping costs, with sector fixed effects interpretable as measure of mean density for different sectors. The existence of per-volume or per-weight shipping cost hence appears as a fact of (second) nature, directly linked to the technical constraints of the transportation sector. In contrast, the notion of per-unit shipping cost is one step removed from technical constraints and the functioning of the transportation sector. While one may argue that the weight per unit is the result of the firm's choice, this can only be partly as firms in a given sector are very likely to have a limited set of options to alleviate the weight per unit of good; in this respect, studying the technical per-volume or per-weight shipping cost is interesting to understand the context in which they operate.

Another argument is illustrated by graph \ref{graph_Share_of_discrete_goods.png}. After computing the share of discrete goods per 5-digit SITC import in 2009, we have used the changing composition of US imports to approximate the share of discrete goods in the total value of US imports on a yearly basis. According to this rough approximation, discrete goods are between 23\% and 52\% of US imports, only over 50\% for two years. Notice that this magnitude of order stands in line with \cite{Lashkaripour_JIE2020}. Switching to a per-unit approach would hence mean a marked reduction in the trade flows coverage, thereby limiting the general scope of our estimates.
...
After considering all the above argument in depth, we decided not switching the paper from the study of price per weight to the study of price per quantity. We then maintained our original empirical strategy covering the whole period 1974-2019 in the revised version, while we extended the online Appendix with this supplementary robustness check. We hope to convince you that this is a relevant choice. \textbf{XXX CHECK RELECTURE TO BE COMPLETED XXX}
