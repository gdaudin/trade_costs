...
Nevertheless, we did not want to stop at this very first argument. We indeed took the referee's remark very seriously and we investigated this question in depth. This was not without difficulty yet, in particular due to data and estimation constraints. At a more fundamental level, we also believe that looking at transport costs on a per-weight basis do add relevant information. We develop each line of argument in what follows.
...
As discussed before, this dramatically increases the computational difficulty of our method (with no substantial different results, see Subsection 3.3.1. in the main paper \textbf{XXX check XXX}).\smallskip
...
Hence, looking at per-kg transport cost can be interpreted straightforwardly as trying to estimate the actual existing shipping costs, with sector fixed effects interpretable as measure of mean density for different sectors. The existence of per-volume or per-weight shipping cost is directly linked to the technical constraints of the transportation sector. In contrast, the notion of per-unit shipping cost is one step removed from technical constraints and the functioning of the transportation sector. The shipping cost per weight is one of the exogenous parameters firms take into account in their production choices, including weight per unit. Accordingly, studying the technical per-volume or per-weight shipping cost is interesting to understand the context in which they operate.
