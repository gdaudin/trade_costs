...
The two productivity thresholds are accordingly linked according to:
\begin{equation}
...
$$\tau+ t\phi^\ast_x  \left( \frac{f}{f_x} \right)^{\frac{1}{1-\sigma}} >1$$

Notice that, due to additive costs ($t>0$), this condition depends on the general equilibrium of the model (through $\phi^\ast_x $) and cannot be imposed ex-ante by an appropriate restriction on the space parameter. We check this condition holds numerically in solving the model.\textbf{A FAIRE ??? JE CROYAIS LE FAIRE MAIS JE NE LE FAIS QUE DANS LE CAS SANS ADDITIFS.}


\subsubsection{Aggregation, firm entry and exit}

\paragraph{Aggregation} The general equilibrium is characterized by a mass $M$ of producing firms in each country associated with a distribution $\mu(\varphi)$ of productivity levels of a subset of $\left(0,\infty\right)$. Given the productivity threshold $\phi^\ast$, $\mu(\varphi)$ is equal to the conditional distribution of $g(\varphi)$ on the subset $\left[\phi^\ast, \infty\right)$:
\begin{eqnarray}
\mu(\varphi) &=& \left\{
\begin{array}{ll}
...
In this first exercise, we contrast the effects of a reduction of international transport costs depending on whether this occurs through a reduction in ad-valorem costs only, or a combined reduction in both the additive and the multiplicative components. Accordingly, we focus on the results reported in Table \ref{tab:resultats_modele}, Column (1) versus (2) for Air, Column (5) versus (6) for Vessel. 

A first conclusion emerges: Whatever the transport mode (and the magnitude of the transport costs decrease accordingly), welfare gains are higher when this reduction is made (partly) through a reduction of additive costs. The effect is substantial: In Air transport, the reduction of 3.4 percentage points of transport costs generates a welfare gain higher by 0.58 percentage point when almost 60\% of such a reduction occurs through the additive component, rather than when it realizes through a decrease of iceberg costs only. In maritime transport, where the decrease in total transport costs is larger (by 6.4 percentage points), the extra welfare gain amounts to 1.12 percentage points when 50\% of the transport cost reduction occurs through the additive component.
