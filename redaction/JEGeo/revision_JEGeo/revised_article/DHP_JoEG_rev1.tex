...
From this, we obtain the following recursive link: $\widehat{\tau}_{ist} = \widehat{\tau}_{is74}\exp(\gamma_t)$. Given the constraint $\tau>0$, we rewrite to get the percentage change between year 1974 and any year $t>1974$:
\begin{equation*}
\Gamma_{ist} = 100.\frac{\widehat{\tau}_{ist}}{\widehat{\tau}_{is74}} = 100.\frac{\widehat{\tau}_{is74}\exp(\gamma_t)}{\widehat{\tau}_{is74}} = 100\exp(\gamma_t)
\end{equation*}
...
\textbf{REPRENDRE AVEC GUILLAUME. ON ESTIME TAU>1 OU TAU>0? CA CHANGE, CF ANCIENNE VERSION EN ITALIQUE}
