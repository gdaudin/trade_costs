...
% \nonumber % Remove numbering (before each equation)
  ...
  \varphi^\ast &=& \frac{1}{f_^{\frac{1}{1-\sigma}}B^{\frac{-1}{1-\sigma}}}
\end{eqnarray*}
...
Following \cite{melitz}, and with $\Omega_d$, $\Omega_x$ the set of varieties respectively produced at Home and imported, one can then rewrite the price index (\ref{eq:CPI}) as:
\begin{eqnarray*}
...
\end{equation}
\noindent with $\left(1-G(\varphi^\ast)$ the probability of successful entry. In the open-economy setting, $\bar{\pi}$ can be expressed as:
...
\paragraph{Summary} Equations (\ref{eq:def_phitilde}), (\ref{eq:def_phitildex}), (\ref{eq:ZCP}), (\ref{eq:link_thresholds}) and (\ref{eq:FEC}) determine a system of interdependent equations in five endogenous variables $\left\{\bar{\pi}, \widetilde{\varphi}, widetilde{\varphi}_x, \varphi^\ast, \varphi^\ast_x \right\}$, from which we can deduce the equilibrium values of the remaining set of variables (as detailed in Appendix \ref{app:theoretical_model}). Specifically, we will study the consequences of additive costs in terms of welfare, which can be shown to be the inverse of the price level as standard in this literature (the price level $P$ being given by Equation (\ref{eq:CPI_vf})).
