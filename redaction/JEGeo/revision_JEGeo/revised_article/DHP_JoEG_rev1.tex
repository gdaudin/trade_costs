...
\section{The role of additive cost: Theoretical insights}
...
\begin{eqnarray*}
\max_{p^x(\varphi)} \pi_x(\varphi) &=& \left(p^x(\varphi) -\frac{\tau}{\varphi}  \right)c^x(\varphi) - tc^x(\varphi) \\
\text{s.t. }&&c^x(\varphi) = \left(\frac{p^x(\varphi)}{P^\ast}  \right)^{-\sigma} \frac{E^\ast}{P^\ast}
\end{eqnarray*}

\noindent where $E^\ast$ and $P^\ast$ refer to nominal expenditures and the price index in the foreign country, $c^x(\omega)$ to the demand for variety $\omega$ from the Foreign household and $p^x(\omega)$ to the price set by the firm producing the variety $\omega$ on the export market. Solving for this yields:
$$p^x(\varphi) = \frac{\sigma}{\sigma-1}\left[\frac{\tau}{\varphi} +t \right]$$
...
$f$ such that all firms produce in autarky (i.e., such that $\varphi^{\ast,a} = \varphi_{\text{min}}$. $f_x$ endogenously revealed such that the initial steady state (with 1974 values of transport costs) implies a share of exporting firms equal to 21\%, based on \textbf{???}, following so the calibration method of \cite{ghironi} or \cite{Irrazabal_2015}.
...
\end{center}
{\parbox[l]{12cm}{ \vspace{4pt}\footnotesize{Notes: TC = Transport Costs. In columns (1), (3), (5) and (7) only ad-valorem costs are considered. In Columns (2), (4), (6) and (8), we model both ad-valorem and additive costs. $t$ is calibrated so that the ratio $t/\widetilde{p}_x$ from the model matches its empirical target, with $widetilde{p}^{\text{fas}}_x$ the average fas price set in place by those domestic firms that export. We theoretically define the export fas price through: $p_x = \tau p^{\text{fas}} +t$}}}
\end{table}
...
Our empirical results point to two important conclusions: First, additive costs represent a non-negligible part of overall international transport costs. Second, total transport costs are strongly reduced over 1974-2019, and the decrease is particularly marked in the additive component in air transport. We exploit the theoretical implications of these empirical findings in two different ways.

First, we run a comparative statics analysis following a reduction in transport costs based on our estimates in 1974 and 2019, by transport mode. Specifically, we contrast two cases: In the first scenario, the reduction in total transport costs is solely attributable to the ad-valorem component (e.g., contrasting Table \ref{tab:calib_TC}, Column (3) to (1) for air transport, and Column (7) to (5) for maritime transport); in the second scenario, we rely of the true estimates of transport costs, with the reduction of total transport costs coming from both additive and ad-valorem components. In this scenario, we contrast Table \ref{tab:calib_TC}, Column (4) to (2) (Air) and Column (8) to (6) (Vessel). Importantly, we run this comparative statics analysis \textit{ceteris paribus}, notably with the same value of the export fixed cost $f_x$ as in the initial equilibrium (see Table \ref{tab:calib_horsTC}.

Second, we take into account the fact that the share of exporting firms has substantially increased since the 21\% value estimated by \textbf{???} in \textbf{YEAR YYY}. According to \textbf{???}, this share roughly amounts to 35\% in \textbf{???}. This can be matched in the model by resetting the (lower) value of the export fixed cost $f_x$ accordingly. In the second exercise, we thus run a comparative statics analysis where we contrast the 2014 steady states (e.g., with the 2019 transport costs values) with the two dimensions of trade liberalization: the reduction of transport costs (as in the first exercise) and the reduction of the export fixed cost, consistent with the increase in the share of exporters from 21\% to 35\%.

The results are reported in Table \ref{tab:resultats_modele}, for Air (Columns (1)-(4)) and Vessel (Columns (5)-(8)) respectively. Specifically, we report the change in total transport costs (expressed in percentage points, and based on the raw numbers reported in Table \ref{tab:calib_TC}), decomposed in its two dimensions (additive and ad-valorem); as well as the welfare change, both in absolute and in relative terms. For each transport mode, the first two columns report the results of reducing transport costs only, maintaining the same value of $f_x$ as in the initial equilibrium (Columns (1)-(2) for Air, (5)-(6) for Vessel), contrasting the case of only ad-valorem costs (Columns (1), (5)) and in presence of transport costs (Columns (2), (6)). In Columns (3)-(4) for Air and (7)-(8), we report the results when both the variable transport costs and the fixed export costs are lower in the final steady state (2019) relative to the initial one (1974).
...
Fixed export cost: &  \multicolumn{2}{|c|}{Constant $f_x$} &  \multicolumn{2}{|c||}{Lower $f_x$} & \multicolumn{2}{|c|}{Constant $f_x$} & \multicolumn{2}{|c}{Lower $f_x$} \\ \hline
...
$\Delta \mathcal{W}$ & 0.0202&	0.0295	&0.036&	0.053	&0.035&	0.052	&0.047&	0.064 \\
$\Delta \mathcal{W}/\mathcal{W}$ (in \%) & 1.232&	1.819&	2.207	&3.248	&2.179	&3.295&	2.872&	4.052   \\
\hline
\end{tabular}
\end{center}
{\parbox[l]{15cm}{ \vspace{4pt}\footnotesize{Notes: TC = Transport Costs. The reduction in transport costs is expressed in percentage points. }}}
\end{table}

\subsubsection{Reducing international transport costs}

In this first exercise, we contrast the effects of a reduction of international transport costs depending on whether this occurs through a reduction in ad-valorem costs only, or a combined reduction in both the additive and the multiplicative components. In Table \ref{tab:resultats_modele}, this relies on the results reported in Column (1) versus (2) for Air, Column (5) versus (6) for Vessel.\medskip

Whatever the transport mode (and the associated magnitude of the transport costs decrease), welfare gains are higher when this reduction is partly achieved through a reduction of additive costs. The effect is substantial: The reduction of 3.4 percentage points of Air transport costs generates a welfare gain higher by 0.58 percentage point when such a reduction occurs through the additive component (accounting for almost 60\% of the total cost decrease), rather than when it realizes through a decrease in iceberg costs only. In maritime transport, where the decrease in total transport costs is larger (-6.4 percentage points), the extra welfare gain amounts to 1.12 percentage points when the transport cost reduction occurs through the additive component (by 50\%) rather than through the iceberg component only.\smallskip
...
To understand the changes induced by the additive costs, let us start from reasoning in the benchmark model with only ad-valorem costs of \cite{melitz}. In this case, the reduction in transport costs induces an increase in selection on the domestic market ($\phi^\ast$ increases, Figure \textbf{XXXX STOP HERE }), but more domestic firms can now export ($\phi_x^\ast$ decreases). Accordingly, the number of domestic active firms reduces, even if a larger number of them is able to export ($M$ decreases but $M_x$ increases). As a whole, the total number of varieties accessible to the consumer reduces ($M_T$ decreases slightly). Everything else equal, this reduces welfare. This is counteracted by a price competitiveness effect. Due to the tougher selection on the domestic market, the less productive firms no longer enter. As a result, the average productivity on the domestic market increases ($\widetilde{\varphi}^\ast$ increases), which pushes the average productivity upwards ($\widetilde{\varphi}^\ast$ and $\widetilde{\varphi}_T$ increase). This price competitiveness effect pushes welfare upwards;

Reduction in additive costs: More negative variety effect
