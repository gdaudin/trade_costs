...
$\Delta t/\widetilde{p}_x^{\text{fas}}$ (in pp) & 0&	-2.0&	0&	-2.0&	0&	-3.3& 0&	-3.3 \\
...
\end{tabular}
\end{center}
{\parbox[l]{15cm}{ \vspace{4pt}\footnotesize{Notes: TC = Transport Costs, expressed in percentage of the average export fas price. The reduction in transport costs is expressed in percentage points. }}}
\end{table}

\paragraph{Reduction in international variable transport costs}

We first consider the effects of a reduction of variable international transport costs. Specifically, we contrast two different environments: In the first scenario ``\`{a} la M\'{e}litz'', only ad-valorem transport costs apply, which consequently reduce. In the second scenario so called ``\`{a} la Hummels'', international transport costs are made of both additive and ad-valorem costs. The overall transport costs decreases then comes from a reduction in both components. The size of the overall transport costs, as well as their reduction, is identical in both scenarii. We calibrate them in our empirical estimates, considering 1974 and 2019 as initial and final values. They are transport-mode specific. In the scenario ``\`{a} la Hummels'', the initial and finale values of each additive and ad-valorem component is also based on our empirical estimates. Importantly, the size of the overall transport costs decrease is kep identical under both scenarii. What differs is the composition of this decrease. The two scenarii for each transport mode are reported in Table \ref{tab:resultats_modele}.\medskip
...
\paragraph{Reduction of the fixed export cost}

The above results underline the importance of the additive component of international trade costs. Trade liberalization still has positive welfare effects, and even larger when it relies on a reduction in additive costs. One might question the robustness of this last result to the type of trade liberalization considered. Indeed, the reduction in the fixed export costs seems empirically relevant to account for the larger share of exporting firms nowadays relative to the 1970s (Based on XXX, from 19\% in XXX to around 35\% in XXX). As underlined by \cite{melitz} \textbf{l'a-t-il MONTRE VRAIMENT ANALYTIQUEMENT? A VERIFIER}, the reduction of the fixed export cost is also welfare-enhancing, in presence of yet only ad-valorem variable trade costs. A natural question then emerges: To what extent is this result modified in presence of additive costs? Specifically, are the welfare gains of a fixed export costs decrease larger in presence of additive costs?

The answer can be found in Table \ref{tab:resultats_modele_fx}, by comparing Columns (B) to (D) for Air, (F) to (H) for Vessel, the difference between these two columns (for each transport mode) being the lower value of the fixed export cost. Here again, the welfare gains of trade liberalization (under the form of a lower fixed export cost) are larger when variable trade costs involve a part of additive costs. \textbf{A FINIR ET A CREUSER}


\begin{table}[htbp]
  \centering
  \caption{Trade liberalization (2): Reduction of the fixed export cost} \label{tab:resultats_modele_fx}
\begin{center}
\begin{tabular}{l||c|c|c|c||c|c|c|c}
\hline
Mode & \multicolumn{4}{|c||}{Air} & \multicolumn{4}{c}{Vessel} \\ \hline
...
\end{tabular}
\end{center}
%{\parbox[l]{15cm}{ \vspace{4pt}\footnotesize{Notes: The reduction in transport costs is expressed in percentage points. }}}
\end{table}
