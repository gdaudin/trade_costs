...
Nevertheless, it remains that the question deserves further investigation. In this section, we thus study the robustness of our results when unit values are built as the ratio between value and quantity of goods per observation. This was not without difficulty yet, in particular due to data and estimation constraints. At a more fundamental level, we also believe that looking at transport costs on a per-weight basis do add relevant information.
