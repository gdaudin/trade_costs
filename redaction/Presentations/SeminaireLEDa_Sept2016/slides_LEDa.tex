...
\begin{itemize}
\footnotesize{
\item[-] Why would it be more expensive to transport one pair of Italian shoes of price 300\euro  than a pair of Spanish espadrilles of price 10\euro?
}
\end{itemize}
...
\item[$\Rightarrow$] The importance of excluding composition effects \vspace{0.1cm}
\begin{itemize}
\footnotesize{
\item[-]\textit{Pure} transport costs have fallen since 1985 (not 1974), by $\simeq$ 40\% \vspace{0.1cm}
}
\end{itemize}
\item[$\Rightarrow$] The importance of including the additive component \vspace{0.1cm}
\begin{itemize}
\footnotesize{
...
\item[$\Rightarrow$] Estimation using non-linear squares \hyperlink{app_method}{\beamergotobutton{More}} \vspace{0.2cm}
...
\end{equation}
\normalsize
\item Estimate \textit{two} equations, for each year over 1974-2013, by transport mode (air/ocean)\vspace{0.1cm}
\begin{itemize}
\item[-] With additive costs included: Equation (\ref{eq:est_equation})  \vspace{0.1cm}
\begin{itemize}
\item[$\Rightarrow$] By year and transport mode, $\simeq$ 800 fixed effects to estimate   \vspace{0.1cm}
\end{itemize}
\item[-] With only ad-valorem costs: Equation (\ref{eq:model_nlI})
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{table}[htbp]
  ...
  \begin{center}
   ...
   \multicolumn{5}{l}{\textit{Ad-valorem term} ($\widehat{\tau}^{adv}$)}\\ \hline
    ...
    Mean &5.6 &4.0 &5.7 & 4.0\\
    Median  &4.4 & 1.9 &4.5 & 2.0 \\  \hline
...
\ln(\widehat{\tau}_{ikt})&=&\delta +\underbrace{\sum_{i \neq \text{AFG}}\alpha_i.\mathbb{1}_i }_{\simeq \ln(\tau_i)}+ \underbrace{\sum_{k\neq \text{011}}\beta_k.\mathbb{1}_k }_{+ \simeq \ln(\tau_k)}+ \underbrace{\sum_{t \neq 1974}\gamma_t.\mathbb{1}_t}_{\text{``Pure'' TC}}+\epsilon_{ikt} \label{eq:compeffects_mult}
\end{eqnarray}
\normalsize
\begin{itemize}
...
\ln(\widehat{t}_{ijt})&=&\ln\left( \underbrace{\prod_{i \neq \text{ARG}}  \alpha_i.\mathbb{1}_i}_{\simeq \ln(t_i)}+\underbrace{\prod_{k\neq \text{011}}\beta_k.\mathbb{1}_k}_{+ \simeq t_k}\right) + \sum_{t \neq 1974}\gamma_t.\mathbb{1}_t+\epsilon_{ijt} \label{eq:compeffects_add}
\end{eqnarray}
\normalsize
...
\item[-] Equation (\ref{eq:compeffects_mult}) estimated using OLS, Equation (\ref{eq:compeffects_add}) using non-linear least squares (by transport mode) \vspace{0.1cm}
\end{itemize}
\end{itemize}
\end{frame}
