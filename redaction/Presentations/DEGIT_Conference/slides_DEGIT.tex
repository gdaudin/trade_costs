...
\begin{itemize}
\item \textbf{Do additive transport costs matter?} \\ \vspace{0.2cm}
...
\item[-] Using the US imports database over 1974-2013 (air/vessel)\vspace{0.1cm}
\end{itemize}
...
\item[$\Rightarrow$] Allows the estimate the levels of both the ad-valorem and the additive trade costs  \vspace{0.1cm}
\begin{itemize}
...
\item[$\Rightarrow$] Estimation using non-linear squares \hyperlink{app_estimation_method}{\beamergotobutton{More}}\vspace{0.1cm}
...
\item[$\ast$] From 5.8\% to 3.2\% in ocean shipping (mean value over the period) \vspace{0.1cm}
...
\begin{itemize}
\item Based on our estimates of additive/multiplicative transport costs (TC) \hyperlink{app_compeffects_strategy}{\beamergotobutton{Comparison with Hummels (2007)}}\vspace{0.1cm}
\begin{itemize}
\item[-] That vary over time ($t$)/product ($k$) /origin country ($i$) (by transport mode): $\widehat{\tau}_{ikt}, \widehat{t}_{ikt}$ \vspace{0.1cm}
\end{itemize}
\item Starting from these values, extract the ``pure'' TC component \hyperlink{app_compeffects_details1}{\beamergotobutton{More details}}\vspace{0.1cm}
\begin{itemize}
...
\item[-] Build a unified measure of ``total'' transport costs (agglomerate the two components), also both fitted and unfitted \vspace{0.1cm}
%\item[-] From which we extract the fitted ``total'' transport cost measure \vspace{0.1cm}
\end{itemize}
\item What explains the decrease of transport costs over time?
\begin{itemize}
\item[-] Is it mostly due to the composition of trade (product quality/weight, less distant countries of origin...) \vspace{0.1cm}
\item[-] or to the ``pure'' transports costs dimension (e.g., handling costs) ?
\end{itemize}
...
\begin{itemize}
\item International transport costs have substantially reduced over the period (1974-2013) \vspace{0.1cm}
\begin{itemize}
...
\end{itemize}
\item \textbf{Not much evidence of a difference between the ``raw'' and the ``pure'' TC components }\vspace{0.1cm}
...
\item[-] See  \hyperlink{app_compeffects_results}{\beamergotobutton{Figure 3}}
...
\hyperlink{slide_estimation_method}{\beamergotobutton{Back to slide}}
\end{frame}
...
\item Than they actually do, see \hyperlink{slide_compeffects_figure}{\beamergotobutton{Figure 2}}
\end{itemize}
