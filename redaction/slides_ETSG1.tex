...
\begin{itemize}
\item Our measure of international transport costs: The difference between the export (fas) price and the import (cif) price \vspace{0.1cm}
\item Database: US Imports of Merchandise database \hyperlink{app_data}{\beamergotobutton{More}}\vspace{0.1cm}
\begin{itemize}
\item[-] The export ($\simeq$ fas) price, $\widetilde{p}$: the price for one kg of merchandise at the country export point \vspace{0.1cm}
...
For both the ad-valorem and the additive components (by year \& transport mode)\vspace{0.1cm}
\begin{itemize}
\item[-] \footnotesize{Average weighted values over 1974-2013, in percent of the export price \hyperlink{app_results_summary}{\beamergotobutton{More}}  } \vspace{0.1cm}
\end{itemize}
\begin{table}[htbp]
  \centering
  \scriptsize{
...
\item[$\star$] An iceberg cost equal 3.2\% and 2.5\% as mean values in vessel and air respectively in presence of additive costs \vspace{0.1cm}
...
\frametitle{Three main findings}
\begin{itemize}
...
\item[-] Compare the transports costs between air and vessel (excluding composition effects) \vspace{0.1cm}
\end{itemize}
