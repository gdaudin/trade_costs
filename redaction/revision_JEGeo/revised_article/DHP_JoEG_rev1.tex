...
This is quite a standard and widespread strategy, as emphasized by \cite{anderson_wincoop_jel}.
...
To do so, denote by $p$ the price per kg of goods paid by the importer (import, or cif price), $\widetilde{p}$ the (per kg) export (or fas) price and $f$ the shipping charge per kg shipped. In accordance with \cite{hummels2010}, assume that shipping charges may depend on the price $\widetilde{p}$, such that per kg shipping charges are written as $f = \widetilde{p}^{1-\beta} X$, where $X$ represents all other elements of transport costs (such as distance, port quality, etc.). In this case, the link between the import, export prices and shipping charges is given by:
...
\paragraph{Obtaining the estimated equation} We follow \cite{Irrazabal_2015} by considering that \textit{i)} both ad-valorem and additive costs are separable between the origin country ($i$) and the product ($k$) dimensions, and \textit{ii)} this separability is in a multiplicative way for the former and an additive way for the latter.
