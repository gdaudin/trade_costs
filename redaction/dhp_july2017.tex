...
\paragraph{Obtaining the unfitted total transport cost index} For each transport mode, we build the ``total'' transport cost based on Equation (\ref{eq:base_estimee}) according to:
...
\paragraph{More on additive costs} In Figure , we report the share of additive costs in total transport costs over time in both Air and Vessel transport, based on the weighted-mean values $\widehat{t}/\widetilde{p}$ and $\widehat{\tau}$ (in similar terms as those reported in Tables \ref{tab:result_air_3d_detail} and \ref{tab:result_ves_3d_detail}, conditional on a year and a transport mode). Precisely, each point in time is obtained as: $\frac{\frac{\widehat{t}}{\widetilde{p}}}{\widehat{\tau}-1+\frac{\widehat{t}}{\widetilde{p}}}$, expressed in percent (i.e., in percent of the export price $\widetilde{p}$).

\begin{figure}[htbp]
\caption{Share of additive costs}
\label{fig:part_cout_additif}
\begin{center}
\includegraphics[width=10cm, height=7cm]{part_cout_additif.pdf}
